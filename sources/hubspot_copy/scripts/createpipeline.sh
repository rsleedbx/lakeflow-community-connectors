...
#   ./createpipeline.sh hubspot_copy_connection                          # Default table: usertable
#   ./createpipeline.sh hubspot_copy_connection "usertable"              # Explicit YCSB workload
...
#
# Defaults:
#   connection_name: $CONNECTION_NAME env var, or "hubspot_copy_connection" if not set
...
if [ -n "$CONNECTION_NAME" ] && [ -z "$1" ]; then
  ...
  CONN="${1:-hubspot_copy_connection}"
fi
...
if [ "$CONN" = "hubspot_copy_connection" ] && [ -z "$1" ] && [ -z "$CONNECTION_NAME" ]; then
