...
AWS_INIT() {
    ...
    # Export AWS account details
    export aws_account_id="${aws_account_id:-$(jq -r '.Account' /tmp/aws_stdout.$$)}"
    export aws_user_arn="${aws_user_arn:-$(jq -r '.Arn' /tmp/aws_stdout.$$)}"
    
    # Get and export default region
...
DBX_INIT() {
    ...
    # Get current user using DBX wrapper
    DB_EXIT_ON_ERROR="PRINT_EXIT" DBX current-user me
    export DBX_USERNAME=$(jq -r '.userName // empty' /tmp/dbx_stdout.$$ 2>/dev/null || echo "")
}
export -f DBX_INIT
