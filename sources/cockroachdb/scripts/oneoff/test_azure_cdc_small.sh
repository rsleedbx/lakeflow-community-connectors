#!/usr/bin/env bash
# Test CockroachDB → Azure Blob CDC with SMALL test table (cdc_test)
# 
# Fast CDC testing with small table:
#   - Table: cdc_test (100 rows, 1 column family)
#   - Initial scan: ~100 events (completes in 5-10 seconds)
#   - Workload: 20 UPDATE operations for CDC testing
#   - Perfect for rapid CDC verification
#
# Usage: ./test_azure_cdc_small.sh [FORMAT] [WORKLOAD] [--force-new]
#   FORMAT: parquet (default) or json
#   WORKLOAD: manual (default) or stats (analyze only)
#   --force-new: Cancel existing changefeeds and create a fresh one
#
# Prerequisites: Run oneoff/setup_test_table.py first to create cdc_test table
...
if [ "$CHANGEFEED_FORMAT" != "parquet" ] && [ "$CHANGEFEED_FORMAT" != "json" ]; then
    ...
    echo "   WORKLOAD: manual (default) | stats"
    echo ""
    echo "   manual: Run 20 UPDATE operations on cdc_test table"
    echo "   stats: Skip workload and analyze existing files only"
    echo ""
    echo "   Prerequisites: Run oneoff/setup_test_table.py first"
    exit 1
fi

if [ "$WORKLOAD_TYPE" != "manual" ] && [ "$WORKLOAD_TYPE" != "stats" ]; then
...
if [ "$SKIP_CREATION" = false ]; then
    ...
    echo "  ℹ️  This ensures CDC events are captured as 'u' (update) not 'c' (snapshot)"
    echo ""
    echo "  📊 Table: 100 rows × 1 column family = ~100 events to scan"
    echo "  ⏱️  Expected time: 5-10 seconds"
    echo ""
...
if [ "$WORKLOAD_TYPE" = "manual" ]; then
    ...
    echo "  Target: 20 UPDATE operations with 200-byte padding"
    echo ""
    ...
    if [ "$ROW_COUNT" -eq 0 ]; then
        echo "  ⚠️  Table is empty! Run setup_test_table.py first"
        echo "     python3 $SCRIPT_DIR/oneoff/setup_test_table.py"
        exit 1
    fi
    
    # Run 20 UPDATEs (one per row, cycling through the 100 rows)
    # Each update adds 200 bytes to ensure we hit the CDC flush threshold quickly
    echo "  Running 20 UPDATE operations..."
    echo ""
    
    for i in {1..20}; do
        ROW_ID=$(( (i % 100) + 1 ))  # Cycle through rows 1-100
        PADDING=$(printf '_%.0s' {1..200})  # 200 character padding
        UPDATE_MARKER="_update${i}${PADDING}"
        
        $PYTHON_CMD "$SCRIPT_DIR/changefeed_helper.py" execute-sql \
            --sql "UPDATE cdc_test SET data = data || '${UPDATE_MARKER}', value = value + 1 WHERE id = ${ROW_ID}" \
            --json "$CRDB_JSON" \
            --commit >/dev/null
    ...
    echo "     - 20 UPDATE operations"
    echo "     - ~200 bytes padding per update"  
    echo "     - Total data: ~4KB (enough to trigger CDC flush with small table)"
    echo ""

else
    echo "  ⚠️  Unknown workload type: $WORKLOAD_TYPE"
    echo "  Small test table only supports 'manual' workload"
    exit 1
fi
...
# Count snapshot files (should be 1 for single column family table)
